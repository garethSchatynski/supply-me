...
session_start();
include 'db.php';
$pagename="Information on Company Registration";
echo "<head>";
...
echo        "  >";
echo        "<a href=getregistersupplier.php class=breadcrumb>Company Registration Information</a>";
...
if (!$fname or !$address or!$country or!$city or !$postcode or !$telno or!$countrycode or !$password1 or !$password2)
{
	echo "<p>Your form is incomplete ";
	echo "<br>Please fill in all the required details ";
	echo "<br>Go back to <a href=registersupplier.php>Register</a>";
}
else
{	
	...
	else	
	{
		...
		$addnewuserSQL="insert into suppliers 
		(supplierName, supplierAddress, supplierCountry, supplierCity, supplierPostCode, supplierTelNo, supplierCountryCode, supplierEmail, supplierPassword)
		values ('".$fname."','".$address."','".$country."','".$city."','".$postcode."',
		...
		if (mysql_errno($conn)==0)
		{
			...
			echo "<div class='col s12 m4 l8'><a href=loginsupplier.php class='btn-large home'>Sign in to your New account</a></div>";
			echo "</div>";
			echo "</div>";
			echo "<p>";
		}
